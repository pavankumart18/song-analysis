...
def get_demucs_sam_pairs(base_dir):
    """
    Finds matching Demucs and SAM folders for each song.
    Returns dict: { "Song Name": { "Demucs": path, "SAM": path } }
    """
    pairs = {}
    path = Path(base_dir)
...
def generate_technical_report():
    ...
    for song, folders in pairs.items():
        entry = { "song": song, "demucs": {}, "sam": {}, "comparison_notes": "" }
    ...
    print(f"Aggregated data for {len(aggregated_data)} songs.")

    # 2. LLM Generation
    system_prompt = """
    You are a Lead Audio Engineer generating a Formal Technical Audit Report.
    
    **Objective**: Evaluate the performance of "Demucs" vs "SAM" for Indian Source Separation & Segmentation.
    
    **Input Data**:
    A JSON list containing:
    - Quantitative Metrics (Segment counts, detection of Pallavi/Interludes) for both models.
    ...
    **Report Requirements**:
    ...
    1. **Executive Summary**: 
       - High-level conclusion. (e.g. "Demucs offers superior stability, while SAM excels in noise suppression but suffers from instrumental confusion.")
    
    2. **Methodology Overview**:
       - Briefly describe the comparison framework (Waveform vs Spectrogram Masking).
    
    3. **Quantitative Performance Metrics**:
       - Create a Markdown Table comparing key stats across the dataset (e.g. Avg Segments Detected, Vocal Detection consistency).
       - Highlight discrepancies (e.g. Does SAM consistently under-segment?).
    ...
    4. **Qualitative Error Analysis (Thematic)**:
       ...
       - **Structural Integrity**: Discuss how these errors affect the final labeling (Pallavi/Charanam).
    ...
    6. **Recommendations**:
       - Final technical recommendation for the production pipeline.
    
    **Tone**: Formal, Objective, Engineering-focused. Avoid "storytelling". Use terms like "Signal-to-Noise Ratio", "False Positives", "Harmonic content".
    """
    ...
    try:
        ...
        out_path = Path(BASE_DIR) / "Technical_Assessment_Report.md"
